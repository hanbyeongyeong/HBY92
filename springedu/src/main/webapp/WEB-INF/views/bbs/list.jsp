...
<div class="container">
    ...
    <h3 class="text-center p-3 mb-3 bg-white font-weight-bolder text-monospace">게시글 목록</h3>
      ...
      <tbody>
	        	...
	        	<c:if test="${bbsdto.bindent > 0}">
	        	 	<img src="/resources/images/icon_reply.gif"/>
						...
						<a class="text-decoration-none text-success" href="/bbs/view?bnum=${bbsdto.bnum }&${pc.makeSearchURL(pc.recordCriteria.reqPage)}">${bbsdto.btitle }</a>
					</td>           						
				...
				<tr style="background-color:white">
					...
					<button id='write' type="button" class="btn btn-sm btn-outline-primary">글쓰기</button>
					</td>
				</tr>
			</tbody>
	</table>	
      ...
      <div class="row">
        ...
        <select name="searchType" id="key1" class="col-sm-3 custom-select custom-select-sm px-1 mr-1">
      	...
      	<input class="col-sm-5 form-control form-control-sm px-1 mr-1" type="search" 
				...
				<button id='btn1' type="button" class="btn btn-sm btn-outline-primary px-2 mx-0">검색</button>
      </div>      
    </form>
  </div>
